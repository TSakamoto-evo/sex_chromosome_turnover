...
int introduce_y_beneficial(std::vector<int>& freqs, std::mt19937& mt,
...
int main(){
  ...
  ofs1 << "index\tmy\tfy\tmy_neo\tfy_neo\tma\tfa\tfemale" << std::endl;
  for(int i = 0; i < 64; i++){
    ofs1 << i << "\t" << geno.my.at(i) << "\t" << geno.fy.at(i) << "\t" << geno.my_neo.at(i)
    << "\t" << geno.fy_neo.at(i) << "\t" << geno.ma.at(i) << "\t" << geno.fa.at(i) << "\t" <<
  ...
  for(int p_index = 0; p_index < static_cast<int>(p_values.size()); p_index++){
    ...
    while(1){
      ...
      while(check == 0){
        ...
        check = introduce_y_beneficial(freqs, mt, geno);
      }
      ...
      reps++;
      if((regi_y_neo_fix >= max_rep && reps % 10000 == 0 && reps >= 100000) || reps == 2000000){
        break;
      }
    }
    ofs2 << n << "\t" << sm << "\t" << hm << "\t" << sf << "\t" << hf << "\t" << r << "\t";
    ofs2 << u << "\t" << v << "\t" << p << "\t" << max_rep << "\t1\t";
    ofs2 << regi_y_neo_fix * 1.0 / reps << "\t" << regi_y_neo_fix << "\t" << reps << std::endl;
  }
